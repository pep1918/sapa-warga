...
exports.updateStatusSurat = async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, message: 'Status surat berhasil diperbarui di database!' });
    } catch (err) {
...
exports.tambahBerita = async (req, res) => {
    const { admin_id, judul, konten, gambar } = req.body;
    if (!admin_id) return res.status(400).json({ error: "Sesi Admin tidak valid!" });

    try {
        const newId = uuidv4();
        // Beri gambar default jika admin tidak memasukkan URL gambar
        const defaultImage = gambar || 'https://images.unsplash.com/photo-1585829365295-ab7cd400c167?w=800&q=80';
        
        await db.query(
            'INSERT INTO portal_berita (id, admin_id, judul, konten, gambar) VALUES (?, ?, ?, ?, ?)',
            [newId, admin_id, judul, konten, defaultImage]
        );
...
exports.updateBerita = async (req, res) => {
    ...
    try {
        await db.query(
            'UPDATE portal_berita SET judul = ?, konten = ?, gambar = ? WHERE id = ?',
